...
(test/gui
 (test-suite
  ...
  (test-suite
   ...
   (test-suite
    "OK"
    (test-equal? "move left in middle, not blocked"               
               (control CTRL-L-EX1 "left")
               (make-tet (make-posn 4 10) (list)))
    (test-equal? "move left in right corner, block on opposite side, not blocked"
               (control CTRL-L-EX2 "left")
               (make-tet (make-posn 9 10) (list (make-posn 2 10))))
    (test-equal? "move right in middle, not blocked"
               (control CTRL-R-EX1 "right")
               (make-tet (make-posn 6 10) (list)))
    (test-equal? "move right on the left block on opposite side, not blocked"
